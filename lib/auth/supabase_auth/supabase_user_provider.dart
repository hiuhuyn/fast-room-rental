...
class FastRoomRentalSupabaseUser extends BaseAuthUser {
  FastRoomRentalSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fastRoomRentalSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FastRoomRentalSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
